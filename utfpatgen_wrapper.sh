...
if [ -n "$dictionary" ] && [ -f "$dictionary" ] && [ "$dictionary" != "/dev/null" ]; then
...
if [ -n "$pre_output" ] && [ "$pre_output" != "/dev/null" ]; then
    rm -f $pre_output
fi
if [ -n "$utfp_dictionary" ] && [ "$utfp_dictionary" != "/dev/null" ]; then
    rm -f $utfp_dictionary
fi